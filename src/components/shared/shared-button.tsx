...
export function SharedButton({
    ...
    const renderLeftIcon = () => {
        ...
        if (typeof leftIcon === 'object' && 'type' in leftIcon) {
            ...
            if (config.type === 'sf-symbol') {
                return (
                    <IconSymbol name={config.name} size={currentSize.iconSize} color={iconColor} />
                );
            }

            if (config.type === 'icon') {
                return <Icon {...config.config} size={currentSize.iconSize} color={iconColor} />;
            }
        }
    ...
    const buttonLabel = accessibilityLabel || (typeof children === 'string' ? children : 'Button');
